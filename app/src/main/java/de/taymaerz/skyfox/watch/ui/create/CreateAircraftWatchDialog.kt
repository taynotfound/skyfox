...
@Composable
fun CreateAircraftWatchDialogHost(
    hex: String? = null,
    note: String? = null,
    vm: CreateAircraftWatchViewModel = hiltViewModel(),
) {
    NavigationEventHandler(vm)
    ErrorEventHandler(vm)

    var inputValue by remember { mutableStateOf(hex ?: "") }
    var commentValue by remember { mutableStateOf(note ?: "") }
    val isValid = inputValue.length > 5

    AlertDialog(
        onDismissRequest = { vm.navUp() },
        title = { Text(stringResource(R.string.watch_list_add_aircraft_title)) },
        text = {
            Column {
                Text(stringResource(R.string.watch_list_add_aircraft_msg))
                Spacer(Modifier.height(16.dp))
                OutlinedTextField(
                    value = inputValue,
                    onValueChange = { inputValue = it },
                    label = { Text(stringResource(R.string.watch_list_add_watch_type_label_aircraft)) },
                    singleLine = true,
